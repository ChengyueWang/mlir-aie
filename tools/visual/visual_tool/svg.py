...
class Tile:
    def __init__(
        ...
        self.row = row  # Store row coordinate
        self.col = col  # Store column coordinate
    ...
    def draw_ic_connections(self, north, south, east, west):
        ...
        if east:
            ...
            y1 = self.y + config.ic_offset + int(config.interconnect_box_height / 2) - 5  # Move 5pt higher
    ...
    def _draw_diagonal_ic(self):
        # Start from the right bottom corner of the NoC (interconnect) box
        i1 = self.x + config.ic_offset + config.interconnect_box_width
        j1 = self.y + config.ic_offset + config.interconnect_box_height
        
        # End at the middle of the left edge of the functional container
        i2 = self.x + config.aie_container_offset_x
        j2 = self.y + config.aie_container_offset_y + (config.aie_container_height / 2)
    ...
    def add_ic_animation(self, diagonal_to_tile=0, diagonal_from_tile=0, north=0, south=0, east=0, west=0, duration=2, delay=0, color="red"):
        ...
        # Diagonal line - going TO a tile (from NoC to tile)
        if diagonal_to_tile == 1:
            ...
            # End at the middle of the left edge of the functional container
            target_x = self.x + config.aie_container_offset_x
            target_y = self.y + config.aie_container_offset_y + (config.aie_container_height / 2)
        ...
        # Diagonal line - going FROM a tile (from tile to NoC)  
        if diagonal_from_tile == 1:
            # Start from the middle of the left edge of the functional container
            i1 = self.x + config.aie_container_offset_x
            j1 = self.y + config.aie_container_offset_y + (config.aie_container_height / 2)
            
            # End at the right bottom corner of the NoC (interconnect) box
            target_x = self.x + config.ic_offset + config.interconnect_box_width
            target_y = self.y + config.ic_offset + config.interconnect_box_height
...
class AieTile(Tile, Image):
    ...
    def __init__(self, col, row, label=False):
        ...
        # Initialize animation paths to prevent AttributeError
        self.path_up_mem_to_aie = ""
        self.path_up_aie_to_mem = ""
        self.path_down_mem_to_aie = ""
        self.path_down_aie_to_mem = ""

    def add_memory_outlines(self):
         self.local_memory_box.add_memory_outlines(self.MAX_BUFFERS)

    def get_aie_svg(self):
        # Get the basic tile structure
        self.aie_svg = self.get_tile_svg() + self.aie_box.box + self.local_memory_box.box
        
        # Add dedicated text area box with same styling as existing containers
        text_area_x = self.x + config.aie_container_offset_x  # Same x position as functional container
        text_area_y = self.y + config.aie_container_offset_y + config.aie_container_height + 3  # Right after functional container
        # Calculate height to extend almost to tile bottom (leave 5px margin)
        text_box_height = config.tile_height - (config.aie_container_offset_y + config.aie_container_height + 3) - 5
        text_box_svg = f'''<rect x="{text_area_x}" y="{text_area_y}" width="{config.aie_container_width}" height="{text_box_height}" 
                         fill="white" stroke="black" stroke-width="1"/>'''
        self.aie_svg += text_box_svg
        
        # Add custom annotations if they exist
        if hasattr(self, 'custom_annotations'):
            self.aie_svg += self.custom_annotations
        
        # Add hide tile overlay AFTER all content (including annotations)
        self.aie_svg += self.hide_tile_svg
    ...
    def add_buffer(self, color, duration=2, start_empty=1, delay=0, color2=""):
        buffer_svg = self.local_memory_box.add_buffer(color, duration, start_empty,
                                                      delay, color2)
        if buffer_svg is not None:
            self.buffers_svg += buffer_svg
...
class MemTile(Tile, Image):
    ...
    def __init__(self, row, col):
        ...
        # Add outer container like AIE tiles
        self.container = Box(
            "mem_tile_container",
            self.index,
            self.x + config.aie_container_offset_x,
    ...
    def get_mem_tile_svg(self):
        ...
        # Add dedicated text area box with same styling as existing containers
        text_area_x = self.x + config.aie_container_offset_x  # Match outer container position
        text_area_y = self.y + config.aie_container_offset_y + config.aie_container_height + 3  # Right after container
        # Calculate height to extend almost to tile bottom (leave 5px margin)
        text_box_height = config.tile_height - (config.aie_container_offset_y + config.aie_container_height + 3) - 5
        text_box_svg = f'''<rect x="{text_area_x}" y="{text_area_y}" width="{config.aie_container_width}" height="{text_box_height}" 
                         fill="white" stroke="black" stroke-width="1"/>'''
        self.tile_svg += text_box_svg
        
        # Add custom annotations if they exist
        if hasattr(self, 'custom_annotations'):
            self.tile_svg += self.custom_annotations
        
        # Add hide tile overlay AFTER all content (including annotations)  
        self.tile_svg += self.hide_tile_svg
    ...
    def add_buffer(self, color, duration=2, start_empty=1, delay=0, color2=""):
        buffer_svg = self.mem_tile_memory.add_buffer(color, duration, start_empty,
                                                     delay, color2)
        if buffer_svg is not None:
            self.buffers_svg += buffer_svg
...
class IfTile(Tile, Image):
    def __init__(self, row, col):
        ...
        self.mem_path_south_y1 = self.y+config.aie_container_offset_y+config.aie_container_height
        #self.mem_path_south_x2 = 0
        self.mem_path_south_y2 = config.tile_height + config.tile_gap  # Extend below the tile
    ...
    def get_if_tile_svg(self):
        ...
        text_box_svg = f'''<rect x="{text_area_x}" y="{text_area_y}" width="{config.aie_container_width}" height="{text_box_height}" 
                         fill="white" stroke="black" stroke-width="1"/>'''
        self.if_svg += text_box_svg
        
        # Add custom annotations if they exist
        if hasattr(self, 'custom_annotations'):
            self.if_svg += self.custom_annotations
        
        # Add hide tile overlay AFTER all content (including annotations)
        self.if_svg += self.hide_tile_svg
    ...
    def draw_external_memory_connections(self, south=0, color="black"):
        ...
        if south:
            path = f'M {self.mem_path_south_x1}, {self.mem_path_south_y1}, \
                l0, {self.mem_path_south_y2}'
            self.external_interface_svg = f'<path id="south_mem_connection{self.index}" stroke="{color}" stroke-width="2" fill="none"\
                d="{path}"\
                />\n'
...
class SystemMemory:
    def __init__(self, x : int = 0, y : int = 0, width : int = 50,
        ...
        self._sysmem = MemoryBox(
            ...
            0,
            self.x,
            self.y + config.tile_height + 35,  # Increased gap to prevent overlap
            self.width,
            config.aie_box_height - 20,  # Reduced height to prevent overlap
            "white",
            stroke_width=1,
            text1="System Memory",
    ...
    def add_buffer(self, color, duration=2, start_empty=1, delay=0, color2=""):
        buffer_svg = self._sysmem.add_buffer(color, duration, start_empty, delay,
                                             color2)
        if buffer_svg is not None:
            self._buffers_svg += buffer_svg
...
class RyzenAiColumn(Image):
    ...
    def __init__(self, rows=4, cols=1, if_tile=1, mem_label: bool = False):
        ...
        # Create compute tiles for all rows and columns
        for j in range(self.rows):
            for i in range(self.cols):
                # Draw AIEs
                self.aie_tiles.append(AieTile(j, i, True))  # Instantiate AIE tiles

        # Set up IC connections for all tiles
        for j in range(self.rows):
            for i in range(self.cols):
                index = j * self.cols + i
                # Draw IC connections
        ...
        # Set up memory connections for all tiles
        ...
        # Initialize j properly for memory and interface tiles
        # Memory and interface tiles come after compute tiles in the visual layout
        
        # Create memory and interface tiles for all columns
        for i in range(self.cols):
            # Memory tile positioning: at row `rows` (below compute tiles)
            mem_row = self.rows
            self.mem_tiles.append(MemTile(mem_row, i))
            mem_index = len(self.mem_tiles) - 1
            self.mem_tiles[mem_index].draw_ic_connections(north=0, south=1, east=0, west=0)

            if self.if_tile:
                # Interface tile positioning: at row `rows + 1` (bottom, below memory)
                if_row = self.rows + 1
                self.if_tiles.append(IfTile(if_row, i))
                if_index = len(self.if_tiles) - 1
                self.if_tiles[if_index].draw_ic_connections(north=0, south=0, east=0, west=0)
                self.if_tiles[if_index].draw_external_memory_connections(south=1, color="black")

        sysmem_width = self.mem_tiles[self.cols - 1].x - self.mem_tiles[0].x + config.tile_width

        # Choose appropriate reference tile for SystemMemory positioning
        if self.if_tiles:
            ref_x = self.if_tiles[0].x
            ref_y = self.if_tiles[0].y
        elif self.mem_tiles:
            ref_x = self.mem_tiles[0].x  
            ref_y = self.mem_tiles[0].y
        else:
            # Fallback if no memory or interface tiles
            ref_x = 0
            ref_y = 0

        self.sysmem = SystemMemory(ref_x, ref_y, sysmem_width, max_buffers=4)
    ...
    def disable_memory_connections(self):
        """Completely disable/hide all memory connections (blue lines)"""
...
class RyzenAiArray(Image):
    ...
    def __init__(self, rows=4, cols=5, ns_mem_connections=False, remove_if_tile=True):
        ...
        # Initialize j properly for memory and interface tiles
        # If no compute tiles (rows=0), start positioning from row 0
        j = self.rows if self.rows > 0 else 0
        
        # Memory tiles are positioned at row `rows` (below compute tiles)
        mem_row = self.rows
        for i in range(self.cols):
            mem_tile = MemTile(mem_row, i)
        ...
        # Interface tiles are positioned at row `rows + 1` (bottom, below memory)
        if_row = self.rows + 1
        for i in range(self.start_col,self.cols):
            self.if_tiles.append(IfTile(if_row, i))
            self.if_tiles[i-self.start_col].draw_ic_connections(north=0, south=0, east=0, west=0)
            self.if_tiles[i-self.start_col].draw_external_memory_connections(south=1, color="black")

        sysmem_width = self.mem_tiles[self.cols - 1].x - self.mem_tiles[0].x + config.tile_width

        # Initialize SystemMemory using appropriate tile positions
        # Use interface tile position if available, otherwise use memory tile position
        if self.if_tiles:
            sysmem_x = self.if_tiles[0].x
            sysmem_y = self.if_tiles[0].y
        elif self.mem_tiles:
            sysmem_x = self.mem_tiles[0].x
            sysmem_y = self.mem_tiles[0].y
        elif self.aie_tiles:
            sysmem_x = self.aie_tiles[0].x
            sysmem_y = self.if_tiles[0].y  # This might error, but it's the original logic
        else:
            # Fallback for completely empty designs
            sysmem_x = 0
            sysmem_y = 0
            
        self.sysmem = SystemMemory(sysmem_x, sysmem_y, sysmem_width, max_buffers=4*self.cols)
    ...
    def update_hidden_tiles(self):
        noc_styling = getattr(self, 'hide_noc_connections_svg', '')
        self.hidden_tiles_svg = self.hide_all_tiles_svg + self.hide_aie_tiles_svg + self.hide_mem_tiles_svg + self.hide_if_tiles_svg + self.hide_mem_connections_svg + self.hide_interconnect_svg + noc_styling
