...
def my_matmul(M, K, N, m, k, n):
    ...
    def device_body():
        ...
        def sequence(A, B, C):
            ...
            #     (m, mtk),  # Size of A (smallest) tile
            #     (1, K // mtk),  # Size of "group" of tiles
            ...
            dma_free_task(*input_task_groups[3])
            # for group_idx in range(num_groups):
            #     dma_await_task(*output_task_groups[group_idx])
            #     dma_free_task(*input_task_groups[group_idx])
            # Multi-group task lists for managing DMA IDs
            # num_groups = (M // m) * (N // n) // (n_aie_cols * n_aie_rows)
