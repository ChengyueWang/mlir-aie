...
// Transform B matrix from row-major to column-major 64x128 blocks with 1x4 groups of 8x8 sub-blocks
// Input: row-major matrix of size height x width
// Output: matrix arranged in column-major 64x128 blocks, with 1x4 groups (8x32 elements each), in 8x4 arrangement
inline std::vector<float> transform_B(const std::vector<float>& input, size_t height, size_t width) {
  ...
  assert(width % 128 == 0 && "Width must be divisible by 128");
  assert(input.size() == height * width && "Input size must match height * width");
  
  std::vector<float> output(height * width);
  
  size_t big_blocks_rows = height / 64;    // Number of 64x128 blocks vertically
  size_t big_blocks_cols = width / 128;    // Number of 64x128 blocks horizontally
  
  // Iterate through 64x128 blocks in column-major order
  for (size_t big_block_col = 0; big_block_col < big_blocks_cols; big_block_col++) {
    for (size_t big_block_row = 0; big_block_row < big_blocks_rows; big_block_row++) {
      ...
      // Within each 64x128 block, we have an 8×4 arrangement of 1×4 groups
        ...
        for (size_t group_row = 0; group_row < 8; group_row++) {      // 8 rows of groups
          ...
          for (size_t j = 0; j < 32; j++) {  // columns within the 8×32 group (32 columns total)
            for (size_t i = 0; i < 8; i++) {   // rows within the 8×32 group (8 rows total)
              ...
              // Calculate which 8×8 sub-block we're in within the 64×128 block
              ...
              size_t src_col = big_block_col * 128 + sub_block_col * 8 + j_in_block;
              size_t src_index = src_row * width + src_col;
              
              // Calculate destination position
              size_t big_block_index = big_block_col * big_blocks_rows + big_block_row;
              size_t group_index = group_col * 8 + group_row;  // 32 groups per 64×128 block (8×4)
              size_t within_group_index = j * 8 + i;           // column-major within the 8×32 group
              size_t dst_index = big_block_index * (64 * 128) + group_index * (8 * 32) + within_group_index;
