...
void matmul_vectorized_bfp16(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
    ...
    if (g_counter == 0){
      ...
      pC = (bfloat16 *) pC_bfp16 + 1 * m * n / 8;
    }else if (g_counter == 2){
      pC = c_bf16_2nd_half;
    }else {
      pC = c_bf16_2nd_half + 1 * m * n / 8;
    }
    ...
    for (int i = 0; i < m * n / (16 * 16); i = i+4) {
      ...
      int block_col = i % (n/16);
        ...
        aie::accum<accfloat, 64> chess_storage(dm2) acc1_data(aie::load_v<64>(pC + 64)) ;
        aie::accum<accfloat, 64> chess_storage(dm3) acc2_data(aie::load_v<64>(pC + 128)) ;
        aie::accum<accfloat, 64> chess_storage(dm4) acc3_data(aie::load_v<64>(pC + 192)) ;
        ...
        aie::block_vector_input_buffer_stream<bfp16ebs8, 64> pA_stream2(converted_A);
        ...
        aie::block_vector_input_buffer_stream<bfp16ebs8, 64> pA_stream3(converted_A);
        ...
        aie::block_vector_input_buffer_stream<bfp16ebs8, 64> pA_stream4(converted_A);
    ...
    if (k_counter == 4096 / 64 / 4){ // // K / k / DIV
      k_counter = 0;
      // transfer second half BF16 to pC_bfp16

      aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pCOut_stream(pC);
      pCOut_stream.seek(128 *128/2 / 64);
      
      bfloat16 * c_bf16_2nd_half_ptr = c_bf16_2nd_half;
      for (int i=0; i<128 *128/ 2 / 64; i++){
        aie::accum<accfloat, 64> acc_data (aie::load_v<64>(c_bf16_2nd_half_ptr));
        c_bf16_2nd_half_ptr += 64;
        pCOut_stream.push(acc_data.template to_vector<bfp16ebs8>());
