...
// // Layout transform function for C matrix: convert from 2x2 8x8 blocks to normal row-major
// // Input: C matrix in 2x2 arrangement of 8x8 blocks (each block in row-major order)
// // Output: C matrix in normal row-major format
// // Expected input layout: 4 consecutive 8x8 blocks representing:
// // [Block 0,0] [Block 0,1] [Block 1,0] [Block 1,1]
// // Each 8x8 block is stored in row-major order within the block
// std::vector<float> layout_transform_C(std::vector<float>& input, int rows, int cols, int L1_block_m, int L1_block_n) {
//   // Ensure dimensions are compatible with 2x2 arrangement of 8x8 blocks  
//   assert(rows % L1_block_m == 0 && "Rows must be divisible by L1_block_m");
//   assert(cols % L1_block_n == 0 && "Cols must be divisible by L1_block_n");
...
//   printf("C layout: L1_rows: %d, L1_cols: %d, L1_block_m: %d, L1_block_n: %d\n", 
//          L1_rows, L1_cols, L1_block_m, L1_block_n);
  
//   int input_idx = 0;
  
//   // Iterate through L1 blocks in row-major order (matching matrix multiplication output order)
//   for (int L1_row = 0; L1_row < L1_rows; L1_row++) {
//     for (int L1_col = 0; L1_col < L1_cols; L1_col++) {
      
//       printf("Processing L1 block (%d, %d)\n", L1_row, L1_col);
      
//       // Within each L1 block, we have 2x2 arrangement of 8x8 blocks
//       // Process in order: [0,0], [0,1], [1,0], [1,1]
//       for (int block_row = 0; block_row < 2; block_row++) {
//         for (int block_col = 0; block_col < 2; block_col++) {
          
//           // Process each 8x8 block (stored in row-major order)
//           for (int row_in_block = 0; row_in_block < 8; row_in_block++) {
//             for (int col_in_block = 0; col_in_block < 8; col_in_block++) {
              
//               // Calculate the position in the final row-major output matrix
//               int output_row = L1_row * L1_block_m + block_row * 8 + row_in_block;
//               int output_col = L1_col * L1_block_n + block_col * 8 + col_in_block;
//               int output_idx = output_row * cols + output_col;
              
//               // Copy from input (2x2 8x8 block layout) to output (row-major)
...
std::vector<float> layout_transform_C(std::vector<float>& input, int rows, int cols, int L1_block_m, int L1_block_n) {
  printf("DEBUG: rows=%d, cols=%d, L1_block_m=%d, L1_block_n=%d\n", rows, cols, L1_block_m, L1_block_n);
  printf("DEBUG: input.size()=%d, expected output size=%d\n", (int)input.size(), rows * cols);
  
  // Check if input has data
  printf("DEBUG: First 10 input values: ");
  for(int i = 0; i < std::min(10, (int)input.size()); i++) {
    printf("%.1f ", input[i]);
  }
  printf("\n");
  
  std::vector<float> output(rows * cols, -999.0f); // Initialize with -999 to see if we're writing
  ...
  printf("DEBUG: L1_rows=%d, L1_cols=%d\n", L1_rows, L1_cols);
          ...
          for (int row_in_block = 0; row_in_block < 8; row_in_block++) {
            for (int col_in_block = 0; col_in_block < 8; col_in_block++) {
              ...
              if (input_idx >= input.size()) {
                printf("ERROR: input_idx %d >= input.size() %d\n", input_idx, (int)input.size());
                return output;
              }
              ...
              if (output_row >= rows || output_col >= cols) {
                printf("ERROR: output bounds - row=%d (max=%d), col=%d (max=%d)\n", 
                       output_row, rows-1, output_col, cols-1);
                return output;
              }
              ...
              if (output_idx >= output.size()) {
                printf("ERROR: output_idx %d >= output.size() %d\n", output_idx, (int)output.size());
                return output;
              }
  ...
  printf("DEBUG: Elements written: %d, input_idx final: %d\n", elements_written, input_idx);
  printf("DEBUG: First 10 output values: ");
...
printf("m = %d, n = %d\n", m, n);
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      std::vector<float> CVec = layout_transform_C(CVec_raw, M, N, m, n);
