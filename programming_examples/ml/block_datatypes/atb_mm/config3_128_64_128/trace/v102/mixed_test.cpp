...
std::vector<float> layout_transform_C(std::vector<float>& input, int M, int N) {
...
int main(int argc, const char *argv[]) {
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    if (do_verify) {
      ...
      auto CVec_raw = bfp16ebs8ToFloat(C_VOLUME, CVecBfp.data(), 0);
      auto CVec = layout_transform_C(CVec_raw, M, N);
