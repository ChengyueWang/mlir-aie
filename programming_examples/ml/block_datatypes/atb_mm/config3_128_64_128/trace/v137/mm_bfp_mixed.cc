...
constexpr int K_Problemsize = 128;
constexpr int DIV = 4;
...
void zero_vectorized_v64bfp16ebs8(bfp16ebs8 *__restrict cOut) {
  ...
  // const bfloat16 *__restrict c_ptr_bf16_end = c_ptr_bf16 + m * DIV * n / 2;
  ...
  // for (int i=0; i<m * DIV * n; i++) {
  //   c_bf16_2nd_half[i] = 2;
  // }


  // // alignas(aie::vector_decl_align) bfloat16 * c_ptr_bf162 = (bfloat16 *) cOut;
  // for (int i=0; i<m * DIV * n / 2; i++) {
  //   c_ptr_bf16[i] = 0.0;
  // }
...
void matmul_vectorized_bfp16(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
    ...
    if (k_counter == (K_Problemsize / k ) * DIV - 1){ // // K / k  *  DIV
      ...
      aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pCOut_stream_first(pC_bfp16);
      pCOut_stream_first.seek(0);

      bfloat16 * pC_bf16; pC_bf16 = (bfloat16 *) pC_bfp16;
      ...
      // aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pCOut_stream(pC_bfp16);
      // pCOut_stream.seek(m *n/2 / 64);
      // pCOut_stream.seek(0);
