...
// std::vector<bfloat16> layout_transform_C(std::vector<bfloat16>& input, int M, int N) {
//   // Ensure dimensions are compatible with 2x2 arrangement of 8x8 blocks  
//   int input_idx = 0;

//   std::vector<bfloat16> output(M * N);

//   // Process the 4 blocks in order: [0,0], [0,1], [1,0], [1,1]
//   for (int block_idx = 0; block_idx < 4; block_idx++) {
//     // Determine which 8x8 block we're processing
//     int block_row = (block_idx >= 2) ? 1 : 0;  // blocks 2,3 are in second row
//     int block_col = (block_idx % 2);           // blocks 1,3 are in second column
    
//     // Process each element within the 8x8 block (row-major within block)
//     for (int row_in_block = 0; row_in_block < 8; row_in_block++) {
//       for (int col_in_block = 0; col_in_block < 8; col_in_block++) {
//         // Calculate the position in the output row-major matrix
//         int output_row = block_row * 8 + row_in_block;
//         int output_col = block_col * 8 + col_in_block;
...
int main(int argc, const char *argv[]) {
  ...
  //   layout_A_2x1_8x8block(A_float, A_transposed, M, K);
  // layout_transpose_1x2_8x8block(B_float, B_transposed, K, N);

  // // std::vector<float> BVec_transformed = transform_B_2x2(BVec, K, N);
  std::vector<float> AVec_transformed = layout_A_2x1_8x8block(AVec, M, K);
  std::vector<float> BVec_transformed = layout_transpose_1x2_8x8block(BVec, K, N);
