  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 16; block_row = block_row + 1) {
      for (int block_col = 0; block_col < n / 16; block_col = block_col + 1) {
        ...
        // aie::accum<accfloat, 64> chess_storage(dm4) acc0_data(pC0In_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm3) acc1_data(pC0In_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm2) acc2_data(pC1In_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm1) acc3_data(pC1In_stream.pop());      
        ...
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex8) loadC0 = pC0In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex9) loadC1 = pC0In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex10) loadC2 = pC1In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex11) loadC3 = pC1In_stream.pop();

        aie::accum<accfloat, 64> chess_storage(dm0) acc0_data_load(loadC0);
        aie::accum<accfloat, 64> chess_storage(dm1) acc1_data_load(loadC1);
        aie::accum<accfloat, 64> chess_storage(dm2) acc2_data_load(loadC2);
        aie::accum<accfloat, 64> chess_storage(dm3) acc3_data_load(loadC3);
        aie::accum<accfloat, 64> chess_storage(dm0) acc0_data = mac_8x8_8x8T(A0_data_bfp, B0_data_bfp, acc0_data_load);
        aie::accum<accfloat, 64> chess_storage(dm1) acc1_data = mac_8x8_8x8T(A0_data_bfp, B1_data_bfp, acc0_data_load);
        aie::accum<accfloat, 64> chess_storage(dm2) acc2_data = mac_8x8_8x8T(A1_data_bfp, B0_data_bfp, acc0_data_load);
        aie::accum<accfloat, 64> chess_storage(dm3) acc3_data = mac_8x8_8x8T(A1_data_bfp, B1_data_bfp, acc0_data_load);
