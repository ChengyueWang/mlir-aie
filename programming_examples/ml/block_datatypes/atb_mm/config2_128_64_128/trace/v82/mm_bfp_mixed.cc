  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 16; block_row = block_row + 1) {
      for (int block_col = 0; block_col < n / 16; block_col = block_col + 1) {
        ...
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex4) loadC0 = pC0In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex5) loadC1 = pC0In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex6) loadC2 = pC1In_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex7) loadC3 = pC1In_stream.pop();
