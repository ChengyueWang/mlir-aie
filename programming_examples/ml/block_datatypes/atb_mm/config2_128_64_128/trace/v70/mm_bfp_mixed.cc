...
void matmul_vectorized_bfp16(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 8; block_row = block_row + 2) {
      ...
      int C_stream_index = block_row * n /8 + block_col;
      int C_stream_index2  = C_stream_index + 1;
      ...
      // pC1In_stream.seek(C_stream_index + n/8);
      ...
      pC1Out_stream.seek(C_stream_index + n/8);

      aie::block_vector_input_buffer_stream<bfp16ebs8, 64> pB_stream(pB);
      pB_stream.seek(block_col * k/8);
        ...
        aie::accum<accfloat, 64> chess_storage(dm4) acc0_data(pC0In_stream.pop_seek(C_stream_index));
        aie::accum<accfloat, 64> chess_storage(dm3) acc1_data(pC0In_stream.pop_seek(C_stream_index + 1));
        aie::accum<accfloat, 64> chess_storage(dm2) acc2_data(pC1In_stream.pop_seek(C_stream_index + n/8));
        aie::accum<accfloat, 64> chess_storage(dm1) acc3_data(pC1In_stream.pop_seek(C_stream_index + n/8 + 1));
        ...
        // aie::accum<accfloat, 64> chess_storage(dm4) acc0_data(aie::zeros<accfloat, 64>());
        // aie::accum<accfloat, 64> chess_storage(dm3) acc1_data(aie::zeros<accfloat, 64>());
        // aie::accum<accfloat, 64> chess_storage(dm2) acc2_data(aie::zeros<accfloat, 64>());
        // aie::accum<accfloat, 64> chess_storage(dm1) acc3_data(aie::zeros<accfloat, 64>());
