...
void matmul_vectorized_different_datatypes(bfloat16 *__restrict pA,
    ...
    aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pA_bfp16_stream(converted_A);
    pA_bfp16_stream.seek(0);
    aie::vector<bfloat16, 64> A_temp;
    // for (int i = 0; i < M*K/(64); i++) {
    //   A_temp = aie::load_v<64>(pA);
    //   pA += 64;
    ...
    for (int i = 0; i < m * n / (16 * 16); i = i+4) {
        ...
        bfloat16 *pC =  pC_curtile + block_row * N * 16 + block_col * 2 * 64; // 64 elements per 8x8 innermost block, 2x2 per block
        ...
        aie::accum<accfloat, 64> chess_storage(dm3) acc2_data(aie::load_v<64>(pC + 0 + N * 8)) ;
        aie::accum<accfloat, 64> chess_storage(dm4) acc3_data(aie::load_v<64>(pC + 64 + N * 8)) ;
