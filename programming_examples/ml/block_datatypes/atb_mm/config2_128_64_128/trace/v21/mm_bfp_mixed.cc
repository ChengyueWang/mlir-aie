  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 8; block_row = block_row + 2) {
      ...
      for (int block_col = 0; block_col < n / 8; block_col = block_col + 2 * 2) {
        ...
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex0) A0_data_bfp = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex1) A1_data_bfp = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex2) B0_data_bfp = pB_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex3) B1_data_bfp = pB_stream.pop();
        ...
        acc0_data = acc0_data_temp.template to_vector<accfloat>();
