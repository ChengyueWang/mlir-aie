...
void matmul_vectorized_bfp16(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
  ...
  for (int run =0; run < run_num ; run ++){
    ...
    aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pCOut_stream(pC);
    pCOut_stream.seek(0);
    aie::block_vector_output_buffer_stream<bfp16ebs8, 64> pCIn_stream(pC);
    pCIn_stream.seek(0);

    // each ouput block contains 2x2 8x8 blocks
    for (int block_row = 0; block_row < m / 16; block_row = block_row + 1) {
      ...
      for (int block_col = 0; block_col < n / 16; block_col = block_col + 1) {
        ...
        // aie::accum<accfloat, 64> chess_storage(dm4) acc0_data(pCIn_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm3) acc1_data(pCIn_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm2) acc2_data(pCIn_stream.pop());
        // aie::accum<accfloat, 64> chess_storage(dm1) acc3_data(pCIn_stream.pop());
        ...
        pCOut_stream.push(acc0_data.template to_vector<bfp16ebs8>());
        pCOut_stream.push(acc1_data.template to_vector<bfp16ebs8>());
        pCOut_stream.push(acc2_data.template to_vector<bfp16ebs8>());
        pCOut_stream.push(acc3_data.template to_vector<bfp16ebs8>());
      }
    }
