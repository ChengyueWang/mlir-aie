...
constexpr int M = 128/4;  constexpr int K = 96;  constexpr int N = 128;
constexpr int m = 128/4;  constexpr int k = 96;  constexpr int n = 128;
constexpr int r = 8;   constexpr int s = 8;   constexpr int t = 8;
  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 16; block_row = block_row + 1) {
      for (int block_col = 0; block_col < n / 16; block_col = block_col + 1) {
        ...
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex0) A0_data_bfp = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex2) A1_data_bfp = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex1) B0_data_bfp = pB_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex3) B1_data_bfp = pB_stream.pop();
        ...
        aie::accum<accfloat, 64> chess_storage(dm2) acc1_data(pC0In_stream.pop());
        aie::accum<accfloat, 64> chess_storage(dm1) acc2_data(pC1In_stream.pop());
        ...
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex4) A0_data_bfp_pong = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex6) A1_data_bfp_pong = pA_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex5) B0_data_bfp_pong = pB_stream.pop();
        aie::block_vector<bfp16ebs8, 64> chess_storage(ex7) B1_data_bfp_pong = pB_stream.pop();
        acc0_data = mac_8x8_8x8T(A0_data_bfp_pong, B0_data_bfp_pong, acc0_data);
        acc1_data = mac_8x8_8x8T(A0_data_bfp_pong, B1_data_bfp_pong, acc1_data);
        acc2_data = mac_8x8_8x8T(A1_data_bfp_pong, B0_data_bfp_pong, acc2_data);
