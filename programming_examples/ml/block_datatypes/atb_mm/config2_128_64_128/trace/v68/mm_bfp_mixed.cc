  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 8; block_row = block_row + 2) {
      ...
      pB_stream.seek(block_col * k/8);
        ...
        aie::accum<accfloat, 64> chess_storage(dm3) acc1_data(pC0In_stream.pop_seek(C_stream_index + n/8));
        aie::accum<accfloat, 64> chess_storage(dm2) acc2_data(pC1In_stream.pop_seek(C_stream_index2));
        aie::accum<accfloat, 64> chess_storage(dm1) acc3_data(pC1In_stream.pop_seek(C_stream_index2 + n/8));
