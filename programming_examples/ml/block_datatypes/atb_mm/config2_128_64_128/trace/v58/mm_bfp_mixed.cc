...
void matmul_vectorized_bfp16(bfp16ebs8 *__restrict pA, bfp16ebs8 *__restrict pB,
  ...
  for (int run =0; run < run_num ; run ++){
    ...
    for (int block_row = 0; block_row < m / 8; block_row = block_row + 2) {
      ...
      // AIE_LOOP_RANGE(8, )
      for (int block_col = 0; block_col < n / 8; block_col = block_col + 2 * 1) {
        ...
        aie::accum<accfloat, 64> chess_storage(dm0) acc0_data = mac_8x8_8x8T(A0_data_bfp, B0_data_bfp, loadC0);
        aie::accum<accfloat, 64> chess_storage(dm1) acc1_data = mac_8x8_8x8T(A0_data_bfp, B1_data_bfp, loadC1);
        aie::accum<accfloat, 64> chess_storage(dm2) acc2_data = mac_8x8_8x8T(A1_data_bfp, B0_data_bfp, loadC2);
        aie::accum<accfloat, 64> chess_storage(dm3) acc3_data = mac_8x8_8x8T(A1_data_bfp, B1_data_bfp, loadC3);
